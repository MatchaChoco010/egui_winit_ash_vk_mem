...
struct App {
    ...
    color_image_allocations: Vec<vk_mem::Allocation>,
    color_image_views: Vec<vk::ImageView>,
    depth_images: Vec<vk::Image>,
    depth_image_allocations: Vec<vk_mem::Allocation>,
    ...
    egui_integration: ManuallyDrop<egui_winit_ash_vk_mem::Integration>,
...
impl App {
    fn new(event_loop: &EventLoop<()>) -> Result<Self> {
        ...
        let allocator = {
            ...
            Arc::new(vk_mem::Allocator::new(&create_info)?)
        };
        ...
        let (
            ...
            for _ in 0..MAX_FRAMES_IN_FLIGHT {
                ...
                let color_image_allocation_info = vk_mem::AllocationCreateInfo {
                ...
                let (color_image, color_image_allocation, _info) = allocator
                    .create_image(&color_image_create_info, &color_image_allocation_info)?;
                ...
                let (depth_image, depth_image_allocation, _info) = allocator
        ...
        let (uniform_buffers, uniform_buffer_allocations) = {
            (0..swapchain_images.len())
                .map(|_| {
                    ...
                    // Reserve buffer
                    let (buffer, buffer_allocation, _info) = allocator
                        .create_buffer(
        ...
        let (temporary_vertex_buffer, temporary_vertex_buffer_allocation, _info) = allocator
            .create_buffer(
                &vk::BufferCreateInfo::builder()
        ...
        // copy vertices data from temporary buffer to vertex buffer
        {
            ...
            allocator
                .destroy_buffer(temporary_vertex_buffer, &temporary_vertex_buffer_allocation)?;
        }
        ...
        let mut egui_integration = ManuallyDrop::new(egui_winit_ash_vk_mem::Integration::new(
            width,
            height,
            window.scale_factor(),
        ...
        unsafe {
            ...
            let uniform_buffer_object_std140 = uniform_buffer_object.as_std140();
            {
                let mapped_memory = self
                    .allocator
                    .map_memory(&self.uniform_buffer_allocations[image_index])?
                    as *mut u8;
                ...
                self.allocator
                    .unmap_memory(&self.uniform_buffer_allocations[image_index])?;
            }
            // Begin render pass
...
impl Drop for App {
    fn drop(&mut self) {
        unsafe {
            ...
            self.allocator
                .destroy_image(self.image, &self.image_allocation)
                .unwrap();
            ...
            self.allocator
                .destroy_buffer(self.vertex_buffer, &self.vertex_buffer_allocation)
                .unwrap();
            ...
            for i in 0..self.depth_images.len() {
                self.allocator
                    .destroy_image(self.depth_images[i], &self.depth_image_allocations[i])
                    .unwrap();
            }
            for i in 0..self.color_images.len() {
                self.allocator
                    .destroy_image(self.color_images[i], &self.color_image_allocations[i])
                    .unwrap();
            }
            self.device
            ...
            for i in 0..self.uniform_buffers.len() {
                self.allocator
                    .destroy_buffer(self.uniform_buffers[i], &self.uniform_buffer_allocations[i])
                    .unwrap();
            }
